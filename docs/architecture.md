...
```
  ...
  │  ┌──────────┐  ①用户消息        │    │  ┌─────────────────────────────────┐    │
  │  │ 消息平台  │ ─────────────────────►│  │ AC自动机(μs) + 正则 + 语义       │    │
  │  │ 蓝信/飞书 │                  │    │  │ 行业模板(40×12) + Auto-Review    │    │
  │  │ 钉钉/企微 │  ⑥Agent回复      │    │  │ 奇点蜜罐 + PII标记 + 信封签名   │    │
  ...
  │  │          │ ◄─────────────────────│                                          │
  │  │ OpenClaw │                   │    │                                         │
  │  │ AI Agent │  ⑤出站响应        │    │  :18444 出站代理                        │
  │  │          │ ─────────────────────►│  ┌─────────────────────────────────┐    │
  │  └──────────┘                   │    │  │ 出站规则(PII/凭据/命令)         │    │
  │       │                         │    │  │ 污染追踪(12 PII) + 逆转引擎    │    │
  │       │ ③Agent调用LLM           │    │  │ block/warn/log 三级策略         │    │
  │       │                         │    │  └─────────────────────────────────┘    │
  │       ▼                         │    │                                         │
  │  ┌──────────┐  ③LLM请求        │    │  :8445 LLM 反向代理                    │
  │  │          │ ─────────────────────►│  ┌─────────────────────────────────┐    │
  │  │ LLM API  │                   │    │  │ ┌── 请求方向 ──┐ ┌── 响应方向 ──┐│    │
  │  │ DeepSeek │  ④LLM响应         │    │  │ │ LLM规则      │ │ LLM规则      ││    │
  │  │ GLM/Qwen │ ◄─────────────────────│  │ │ 工具策略     │ │ IFC Hide     ││    │
  │  │          │                   │    │  │ │ 执行计划     │ │ 偏差检测     ││    │
  │  └──────────┘                   │    │  │ │ Capability   │ │ 反事实验证   ││    │
  │                                 │    │  │ │ IFC检查      │ │ 污染逆转     ││    │
  │                                 │    │  │ │ 蜜罐注入     │ │ Canary检测   ││    │
  ...
  └─────────────────────────────────┘    │  :9090 管理层                           │
                                         │  ┌─────────────────────────────────┐    │
  完整业务流:                            │  │ Dashboard(50页面) + API(487路由) │    │
  ①用户→消息平台→龙虾卫士→OpenClaw       │  │ SQLite(67表) + 安全画像(5维)    │    │
  ②                                     │  │ WSS RPC → Gateway远程管理(88方法)│    │
  ③OpenClaw→Agent→龙虾卫士→LLM API      │  │ 多租户 · 会话回放 · 报告 · 红队  │    │
  ④LLM响应→龙虾卫士→Agent               │  └─────────────────────────────────┘    │
  ⑤Agent→OpenClaw→龙虾卫士→消息平台API   │                                         │
  ⑥回复送达用户                          │  横切面引擎(贯穿全链路):                 │
                                         │  · 行为画像 + 攻击链 + 路径策略          │
  龙虾卫士 = 安全侧车                    │  · 信封签名 + Merkle验证 + 事件总线      │
  不改业务流，只在链路上                  │  · 自进化 + 语义检测 + 安全画像          │
  每个环节注入安全检测                    │  · 审计日志 + trace_id 全链路关联        │
                                         └─────────────────────────────────────────┘
```
