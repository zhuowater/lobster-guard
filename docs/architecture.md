...
```
  ...
  ┌──────────┐  ─────────────────────────► │  ┌────────────────────────────────────┐    │
  │          │                            │  │ AC自动机 + 正则 + 行业模板(40×12)   │    │
  │ 消息平台  │               ⑥送达回复    │  │ Auto-Review + 语义 + 奇点蜜罐      │    │
  │ 蓝信/飞书 │  ◄───────────────────────── │  │ PII标记 + 信封签名 + 事件发射      │    │
  │ 钉钉/企微 │                            │  │ 四级动作: block/review/warn/log    │    │
  │          │                            │  └──────────────┬─────────────────────┘    │
  └──────────┘                            │        ②通过     │ 拦截→拒绝                │
                                          │                  ▼                          │
                                          │        路由表(亲和绑定) → 上游容器池         │
  ...
  │          │                            │                                             │
  │ OpenClaw │                            │                                             │
  │ AI Agent │               ③LLM请求     │  :8445 LLM 反向代理                         │
  │          │  ─────────────────────────► │  ┌────────────────────────────────────┐    │
  │          │                            │  │ ┌── 请求方向 ───┐ ┌── 响应方向 ───┐ │    │
  │          │               ④LLM响应     │  │ │ LLM规则       │ │ LLM规则       │ │    │
  │          │  ◄───────────────────────── │  │ │ 工具策略      │ │ IFC Hide      │ │    │
  │          │                            │  │ │ 执行计划      │ │ 偏差检测      │ │    │
  │          │               ⑤Agent回复   │  │ │ Capability    │ │ 反事实验证    │ │    │
  │          │  ─────────────────────────► │  │ │ IFC检查       │ │ 污染逆转      │ │    │
  └──────────┘                            │  │ │ 蜜罐注入      │ │ Canary检测    │ │    │
                                          │  │ └───────────────┘ └───────────────┘ │    │
                                          │  │ SSE streaming 安全扫描               │    │
  ...
  │ LLM API  │                            │  :18444 出站代理                             │
  │ DeepSeek │                            │  ┌────────────────────────────────────┐    │
  │ GLM/Qwen │                            │  │ 出站规则(PII/凭据/命令)            │    │
  │          │                            │  │ 污染检查(CheckOutbound, 带标签→拦截)        │    │
  └──────────┘                            │  │ block/warn/log 三级策略             │    │
                                          │  └──────────────┬─────────────────────┘    │
                                          │        ⑥通过     │ 拦截→脱敏/阻断           │
                                          │                  ▼                          │
                                          ...
                                          │  横切面引擎(贯穿全链路):                     │
                                          │  行为画像 · 攻击链 · 路径策略 · 信封签名     │
                                          │  Merkle验证 · 事件总线 · 自进化 · 安全画像   │
                                          │  审计日志 · trace_id 全链路关联              │
                                          └─────────────────────────────────────────────┘

  业务流: ① 消息平台→龙虾 ② 龙虾→OpenClaw ③ OpenClaw→龙虾→LLM ④ LLM→龙虾→OpenClaw ⑤ OpenClaw→龙虾 ⑥ 龙虾→消息平台
  关键: 三个业务实体互相不直连，所有流量必经龙虾卫士
```
