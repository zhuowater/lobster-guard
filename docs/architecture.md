...
左边：**业务实体**（消息平台 · OpenClaw · LLM API）— 不感知龙虾卫士的存在
右边：**龙虾卫士**透明嵌入每个连接点，所有流量必经
...
```
  ...
  │  ┌──────────┐        │     │  :18443 入站代理                                      │
  │  │ 消息平台  │ ──────────►  │  ┌──────────────────────────────────┐                │
  │  │ 蓝信/飞书 │  ①回调  │     │  │ AC自动机 + 正则 + 行业模板(40×12) │                │
  │  │ 钉钉/企微 │        │     │  │ Auto-Review + 语义 + 奇点蜜罐    │                │
  │  └──────────┘        │     │  │ PII标记 + 信封签名 + 事件发射     │                │
  ...
  │  │ AI Agent │  ⑤回复  │     │  │ 污染追踪(12 PII) + 逆转引擎     │                │
  │  └──────────┘        │     │  │ block/warn/log 三级策略          │                │
  │                      │     │  └───────────────┬──────────────────┘                │
  │  ┌──────────┐        │     │         ⑥通过     │ 拦截→脱敏/阻断                    │
  │  │ 消息平台  │ ◄──────────── │                   ▼                                  │
  │  │ (API)    │  ⑥送达  │     │         调用消息平台 API 发回                          │
  │  └──────────┘        │     │                                                       │
  │                      │     │  :9090 管理层                                         │
  │  ┌──────────┐        │     │  ┌──────────────────────────────────┐                │
  │  │ 用户收到  │        │     │  │ Dashboard(50页面) + API(487路由)  │                │
  │  │ 回复     │        │     │  │ SQLite(67表) + 安全画像(5维)     │                │
  │  └──────────┘        │     │  │ WSS RPC → Gateway远程管理         │                │
  │                      │     │  │ 多租户 · 会话回放 · 报告 · 红队   │                │
  └──────────────────────┘     │  └──────────────────────────────────┘                │
                               │                                                       │
  完整业务流:                  │  横切面引擎(贯穿全链路):                               │
  ① 消息平台回调 → 龙虾卫士   │  · 行为画像 + 攻击链 + 路径策略                        │
  ② 入站检测通过 → OpenClaw   │  · 信封签名 + Merkle验证 + 事件总线                    │
  ③ Agent→龙虾卫士→LLM(请求)  │  · 自进化 + 语义检测 + 安全画像                        │
  ④ LLM→龙虾卫士→Agent(响应)  │  · 审计日志 + trace_id 全链路关联                      │
  ⑤ Agent回复→龙虾卫士出站检测 │                                                       │
  ⑥ 检测通过→消息平台API→用户 │                                                       │
                               │  关键: 消息平台不直连OpenClaw                          │
  龙虾卫士 = 透明安全侧车      │  OpenClaw不直连LLM API                                 │
  业务实体互相不直连           │  所有流量必经龙虾卫士3个端口                            │
  所有连接都经过龙虾卫士       └───────────────────────────────────────────────────────┘
```
