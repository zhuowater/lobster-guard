          ...
          >
            ...
            <div class="tl-content">
              ...
              <template v-else-if="ev.type === 'llm_call'">
                <div class="tl-detail tl-llm-head" @click="ev._expanded = !ev._expanded" style="cursor:pointer">
                  ...
                  <span class="tl-expand-icon">{{ ev._expanded ? '▾' : '▸' }}</span>
                </div>
                <!-- Expanded request/response content -->
                <div v-if="ev._expanded" class="llm-preview-content">
                  <div v-if="ev.request_preview" class="llm-preview-block llm-preview-req">
                    <div class="llm-preview-label">Request</div>
                    <pre class="llm-preview-body">{{ ev.request_preview }}</pre>
                  </div>
                  <div v-if="ev.response_preview" class="llm-preview-block llm-preview-resp">
                    <div class="llm-preview-label">Response</div>
                    <pre class="llm-preview-body">{{ ev.response_preview }}</pre>
                  </div>
                  <div v-if="!ev.request_preview && !ev.response_preview" class="llm-no-content">
                    内容未记录（旧数据）
                  </div>
                </div>
...
/* LLM Content Preview (v18.0) */
.tl-llm-head { cursor: pointer; user-select: none; }
.tl-expand-icon { font-size: 11px; color: var(--text-tertiary); margin-left: 4px; }
.llm-preview-content { margin-top: var(--space-2); }
.llm-preview-block { margin-bottom: var(--space-2); border-radius: var(--radius-md); overflow: hidden; }
.llm-preview-label {
  font-size: 10px; font-weight: 600; padding: 3px 10px;
  text-transform: uppercase; letter-spacing: 0.05em;
}
.llm-preview-req .llm-preview-label { background: rgba(99,102,241,0.15); color: #818cf8; }
.llm-preview-resp .llm-preview-label { background: rgba(34,197,94,0.1); color: #4ade80; }
.llm-preview-body {
  font-size: 12px; padding: 8px 10px; margin: 0;
...
.llm-no-content {
  font-size: 12px; color: var(--text-tertiary); font-style: italic; padding: 8px;
}
</style>
