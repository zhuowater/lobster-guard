<template>
<div class="page"><div class="page-header"><div><h1 class="page-title"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> Plan Deviation Detector</h1>
<p class="page-subtitle">Detect and auto-repair deviations between actual tool calls and execution plan templates</p></div>
...
<div class="tab-bar">
<button class="tab-btn" :class="{active:tab==='list'}" @click="tab='list'">Deviations ({{ deviations.length }})</button>
<button class="tab-btn" :class="{active:tab==='config'}" @click="tab='config'">Configuration</button>
</div>

<div v-if="tab==='list'" class="section">
<table class="data-table" v-if="deviations.length"><thead><tr><th>ID</th><th>Type</th><th>Tool</th><th>Severity</th><th>Decision</th><th>Repaired</th><th>Trace</th></tr></thead>
...
<td>{{ d.repaired ? 'Yes' : '-' }}</td>
<td class="mono">{{ (d.trace_id||'').substring(0,12) }}</td></tr></tbody></table>
<div v-else class="empty">No deviations detected</div>
</div>

<div v-if="tab==='config'" class="section">
<div class="config-form">
<div class="config-row"><label>Enabled</label><label class="toggle"><input type="checkbox" v-model="configForm.enabled"/><span class="slider"></span></label></div>
<div class="config-row"><label>Auto-Repair</label><label class="toggle"><input type="checkbox" v-model="configForm.auto_repair"/><span class="slider"></span></label></div>
<div class="config-row"><label>Max Repairs per Trace</label><input v-model.number="configForm.max_repairs" type="number" class="field-input" min="0"/></div>
...
export default {
  ...
  computed: {
    statCards() { const s = this.stats; return [
      {label:'Total Checks', value: s.total_checks??0, color:'#6366F1'},
      {label:'Deviations', value: s.total_deviations??0, color:'#F59E0B'},
      {label:'Critical', value: s.critical_count??0, color:'#EF4444'},
      {label:'Repairs', value: s.repairs_applied??0, color:'#10B981'}
    ]}
  },
