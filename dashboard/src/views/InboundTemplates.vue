<template>
  <div>
        ...
        <div v-if="expandedIds[tpl.id]" class="template-rules">
          ...
          <div v-for="(rule, idx) in (tpl.rules || [])" :key="idx" class="rule-item">
            <div class="rule-header">
              ...
              <span v-if="rule.group" class="rule-dir-tag">{{ rule.group }}</span>
            </div>
            ...
            <div v-if="rule.message" class="rule-desc-text">提示: {{ rule.message }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="filteredTemplates.length === 0 && !loading" class="empty-state card">
      ...
      <div class="empty-text">暂无匹配的入站规则模板</div>
    </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content modal-lg">
        <div class="modal-header">
          <h3>{{ isEdit ? '编辑入站规则模板' : '创建入站规则模板' }}</h3>
          <button class="modal-close" @click="closeModal">&times;</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>模板 ID</label>
            <input class="form-input" v-model="form.id" :disabled="isEdit" placeholder="如 my-inbound-tpl" />
          </div>
          <div class="form-group">
            <label>名称</label>
          ...
          <div class="form-group">
            ...
            <textarea class="form-input rules-json-editor" v-model="form.rulesJSON" rows="12"
              placeholder='[{"name":"规则1","patterns":["关键词1","关键词2"],"action":"block","category":"pii","type":"keyword","priority":10}]'></textarea>
...
async function submitForm() {
  ...
  try {
    ...
    if (isEdit.value) {
      await apiPut('/api/v1/inbound-templates/' + form.value.id, body)
      showToast('模板已更新', 'success')
    } else {
      await apiPost('/api/v1/inbound-templates', body)
...
async function doDelete() {
  ...
  try {
    await apiDelete('/api/v1/inbound-templates/' + deleteTarget.value.id)
