...
func TestInboundProxy_RateLimit_Webhook(t *testing.T) {
	...
	inbound := NewInboundProxy(cfg, gp, engine, logger, pool, routes)
...
func TestHealthz_RateLimiter(t *testing.T) {
	...
	routes := NewRouteTable(db, false)
	gp := NewGenericPlugin("", "content")
	inbound := NewInboundProxy(cfg, gp, NewRuleEngine(), logger, pool, routes)
	mgmt := NewManagementAPI(cfg, "", pool, routes, logger, outboundEngine, inbound)
