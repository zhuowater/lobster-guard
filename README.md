...
| 能力 | 说明 |
|------|------|
| 🛡️ **智能检测** | AC 自动机 + 正则 + 检测链 Pipeline + 可选 LLM 语义分析 · 4 场景规则模板库（66 条）|
| 🧠 **上下文感知** | 会话级风险积分 · 多轮对话攻击识别 · 自动升级（warn → block）|
| 🔒 **出站拦截** | 防止 Agent 泄露身份证号、API Key、私钥等敏感信息 · PII 正则可配置 |
...
| 📊 **全量审计** | SQLite 持久化 · CSV/JSON 导出 · 自动归档 · 时间线图 · 全文搜索 |
| 🚨 **告警通知** | Block 事件 webhook 实时推送 · 蓝信/通用格式 |
| 📈 **可观测性** | Prometheus 指标 · 结构化日志 · trace_id 全链路追踪 · 实时监控大屏 |
| 🖥️ **管理后台** | 60KB 内置 Dashboard · go:embed · 路由/用户/规则/审计全面管理 |
| 💾 **高可用** | 优雅关闭 · 5 维健康检查 · 数据备份/恢复 · Store 抽象层 |
| ⚡ **限流** | 令牌桶 · 全局/每用户 · 白名单 · 429 + Retry-After |

### 🏗️ 设计哲学

- **单二进制部署** — 24 个源文件编译出一个二进制（含 Dashboard + 规则模板），扔上去就跑
- **Fail-Open** — 检测异常不阻塞业务，宁可漏检不可误杀
- **零外部依赖** — 只依赖 Go 标准库 + SQLite + YAML 解析 + WebSocket，不引入 Redis/MQ/K8s
...
| 功能 | 说明 |
|------|------|
| 📊 系统状态 | 版本、运行时间、5 维健康检查（DB/上游/磁盘/内存/goroutines），每 10 秒刷新 |
| 🔗 上游管理 | 容器列表，绿色脉冲 = 健康，红色闪烁 = 异常 |
| 🗺️ 路由管理 | 用户→容器绑定，Bot/部门筛选，批量操作，姓名/邮箱/手机自动显示 |
| 📈 统计面板 | 总请求/拦截/告警大数字 + CSS 柱状图 |
| ⏱️ 限流统计 | 全局/每用户限流状态 |
| 🎯 规则命中率 | 按规则排行 + 分组标签 |
| ⚙️ 规则管理 | 入站/出站规则查看 + 一键热更新 + 正则/关键词类型 + group 彩色标签 |
| 📋 审计日志 | 方向/动作/发送者/App ID/关键词筛选 + trace_id + 24h 时间线图 + CSV/JSON 导出 |
| 📡 实时监控 | QPS 柱状图 + 攻击实时流 + 关键指标大字，每 3 秒刷新 |
| 🌐 WebSocket | 活跃 WebSocket 连接列表 |

Dashboard 是一个 **60KB 的单 HTML 文件**（go:embed 嵌入二进制），零外部依赖，无需 npm/webpack。
...
| GET | `/api/v1/rule-bindings` | 查看 app_id 规则组绑定 |
| POST | `/api/v1/rule-bindings/test` | 测试某 app_id 会应用哪些规则 |
...
| 请求追踪 | 5+ | trace_id 生成/header 传递/审计记录 |
| 结构化日志 | 5+ | text/json 格式/字段输出 |
...
├── dashboard.html          # 管理后台 (~60KB 单 HTML)
