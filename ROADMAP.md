...
**当前状态**: v7.2.0 · Go 24 文件 ~11,500 行 · Vue 26 文件 ~4,200 行 · 472+ 测试 · 3 个依赖
...
### v8.0 — 运维工具箱
> 一站式运维中心，新增"运维"页面（/ops），4 个 Tab

- [ ] 配置查看器（YAML 语法高亮 · 脱敏展示 · 复制按钮）
- [ ] 备份管理（创建 / 列表 / 下载 / 删除 / 恢复 全生命周期）
- [ ] 系统诊断（上游连通性测试 · 规则性能统计 · 实时日志查看器）
- [ ] 告警设置（webhook 配置 · 告警规则 · 告警历史）
- [ ] Sidebar 新增"运维"导航项
- [ ] 后端新增 API：config/view · system/diag · alerts/history
...
v7:    Dashboard 品质（UI打磨 + 配色升级 + 交互改造 + 数据体验）✅ 完成
v8:    运维工具箱（配置 + 备份 + 诊断 + 告警）← 进行中
