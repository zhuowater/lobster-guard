...
- [ ] v11.0 **攻击者画像**
  - 用户风险评分模型（基于历史行为：注入尝试频率 · 高危工具使用率 · 异常时段活跃度）
  - 用户行为时间线（瀑布图：每次交互的类型、风险等级、工具调用）
  - 风险用户 TOP10 榜单（Dashboard 卡片）
  - 用户详情页（点击用户名跳转，完整行为历史 + 风险趋势）
...
- [ ] v12.0 **报告模板引擎**
  - 内置 3 种模板：安全日报 · 周报 · 月度审计报告
  - 自动数据聚合（拦截趋势 · 攻击类型分布 · 成本分析 · 高风险事件 · 规则有效性）
  - HTML 报告（内联 CSS，邮件友好）
  - 定时生成 + webhook 推送
  - Dashboard "报告"页面：历史报告列表 · 在线预览 · 一键生成
...
v10:    LLM 规则引擎 + Canary Token + Shadow Mode ← 进行中
v11:    攻击者画像 + 异常基线检测
v12:    合规报告 + 审计导出
v13:    会话回放 + Prompt 追踪
v14:    多租户 + 企业权限
