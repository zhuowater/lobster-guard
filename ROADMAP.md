...
> **当前版本：v36.5** · 364+ commits · Go ~68,000 行 + 测试 ~37,500 行 · Vue ~32,700 行 · 1252+ 测试 · 491 API · 50 页面 · 75 Vue 组件 · 5 依赖
>
> 更新时间：2026-04-08
...
> - ✅ **CaMeL** (Google, arXiv:2503.18813) — 控制流/数据流分离，设计上消灭注入 → **v25 已完成**
> - ✅ **AttriGuard** (arXiv:2603.10749) — 因果归因，反事实检验 tool call 来源 → **v24 已完成**
> - ✅ **Runtime Governance** (arXiv:2603.16586) — 路径级策略，执行历史感知 → **v23 已完成**
> - **Fides/IFC** (Microsoft, arXiv:2505.23643) — 信息流控制，变量级污点追踪 → v26 待开发
>
> 架构演进策略: **先叠加、后重构**
> v23-v25 ✅ 在现有三条数据通道上叠加学术成果（不需要上下游改造）— 已全部完成
> v26-v28 需要 MCP Proxy + Agent 身份协议
> v29+ 企业级 + 生态

### v23.x — 路径级策略引擎 · 上下文感知安全决策 ✅ DONE
...
- [x] v25.0 **执行计划编译器（PlanCompiler）** ✅ (commit b3f33d1, 2026-03-26) 🔥🔥
  - **核心架构变革**:
    ```
...
- [x] v25.1 **Capability 权限系统** ✅ (2026-03-26)
...
- [x] v25.2 **Plan 偏差检测 + 自动修复** ✅ (2026-03-26)
...
### v30.x — 质量硬化 · 模板统一 · AC 智能分级 · 架构治理 ✅ DONE
...
- [x] v30.1 **AC 智能分级 + LLM 复核** 🔥 ✅ (commit 2de9dba, 2026-03-29)
...
- [x] v30.2 **api.go 架构拆分** ✅ (commit 6de4474, 2026-03-29)
...
- [x] v30.3 **引擎链路修复** ✅ (commit 6de4474, 2026-03-29)
...
- [x] v30.4 **Dashboard 系统设置中心** ✅ (commit 6de4474, 2026-03-29)
...
### v31.x — 统一行业模板 · PII 整合 · LLM 自动复核 · SSE 流式拦截 ✅ DONE
> 40 行业 × 三维度（入站+LLM+出站）一键启用全链路保护
> 代码默认规则 = benchmark 优化后的完整规则集（新部署无需 conf.d）

- [x] v31.0 **统一行业模板系统** ✅ (commits fa06717→a9eb35b, 2026-03-29)
  - `IndustryTemplate` 结构：每个行业一个模板，包含 `inbound_rules` + `llm_rules` + `outbound_rules`
  - 40 个行业模板，12 分类（科技/金融/医疗/政务/服务/媒体/交通/工业/能源/国防/教育/合规）
  - 全局 enable/disable + 租户绑定（并集不冲突）
  - 入站规则 merge：conf.d 与代码默认合并（同名覆盖，不同名追加）
  - conf.d 24 规则 243 patterns 全部同步到 `getDefaultInboundRules()` 代码默认
  - `regex_role_injection` 3 条正则规则加入代码默认
  - Auto-review 扩展到 7 条高 FP 规则
  - Benchmark: 500 样本 AC-only F1=0.936 → AC+LLM 二审 F1=1.000（零误报零漏检）
  - Dashboard: Rules.vue 行业模板管理 UI（分类筛选 + Toggle 启用 + 规则详情）

- [x] v31.1 **PII 整合 + LLM 自动复核 + SSE 流式拦截** ✅ (commits 9b4faca+eebea19, 2026-03-29)
  - 3 条 PII 正则规则加入 `getDefaultInboundRules()`，`DetectPII()` shim 从命中结果填充 `r.PIIs`
  - `LLMRuleEngine` 集成 `autoReviewMgr`：LLM 代理的 block 路径也走自动复核
  - SSE 流式实时拦截：滑动窗口 512 字节 AC 检测，流式响应中实时发现并拦截违规内容
...
#### 功能补全（v32.10-v32.15，原 v31 规划并入）

- [x] v32.10 **出站行业模板** — ~~不需要~~ 出站规则是 regex PII 检测（手机号/身份证/银行卡/私钥等），**数据格式全行业通用**，无需按行业拆分。142 已有 9 条出站规则在生产运行。

- [x] v32.11 **红队→规则演化闭环**
  - 红队引擎（v14.2）发现的绕过模式 → 自动生成新规则**建议**（不自动添加）
  - Dashboard: 规则建议队列（审核→接受/拒绝→热更新到 RuleEngine）
  - 演化引擎（v19）与红队引擎对接：红队报告 → 演化建议 → 管理员审批 → 规则生效
  - 闭环：攻击→检测→绕过→红队发现→建议→审批→加固→再测

- [x] v32.12 **Merkle 审计验证 UI**
  - `merkle.go` 已实现 Merkle 树批量签名（v18.0），Dashboard 缺验证入口
  - Envelopes.vue "链验证" Tab 增强：选择时间范围 → 批量验证 → 展示完整性报告
  - 异常标红：被篡改的审计记录高亮展示
  - 导出验证报告（HTML），支持合规审计场景

- [x] v32.13 **金丝雀令牌自动轮换**
  - 当前 `canary_token` 在 config 中写死，一旦泄露攻击者可以规避
  - 自动轮换机制：每 N 小时（可配）自动生成新的 canary token + 写回 config
  - 轮换日志：记录每次轮换时间 + 旧 token 泄露检测结果
  - Dashboard: Settings 安全 Tab 金丝雀管理卡片（当前 token 状态、上次轮换、泄露检测历史）

- [x] v32.14 **报告定时自动生成**
  - `report.go`（v12.0）能手动生成报告，但没有 cron 触发
  - 新增 `report_scheduler.go`：可配定时任务（每日/每周/每月）
  - 报告内容：安全事件摘要 + 健康分趋势 + 规则命中 TOP10 + 建议
  - 与告警通知（v5）复用 Webhook 通道（蓝信群推送）
  - Dashboard: Reports.vue 增加定时配置 + 历史列表

- [x] v32.15 **CaMeL 三引擎 Dashboard 独立开关**
...
- [x] **v36.4 LLM Proxy 管线化重构** ✅ (2026-04-08)
  ...
  - 当前进展：已抽出 `src/llm_request_preprocess.go`，先收拢 trace/taint/request-path 三个 request preprocess helper；新增 `src/llm_request_policy.go` 收口请求侧规则检测与立即阻断分支；新增 `src/llm_upstream_forward.go` 收口上游请求构造、header 复制、审计上下文构建；新增 `src/llm_response_policy.go` 收口非流式 + SSE 缓冲模式的响应规则阶段；新增 `src/llm_stream_finalize.go` 收口 SSE 尾部 rewrite/event formatting helper；新增 `src/llm_observability.go` 收口 envelope sealing 与 audit async hooks；新增 `src/llm_response_postprocess.go` 收口 taint propagation / reversal / cache-store 响应后处理 helper；新增 `src/llm_tool_governance.go` 收口非流式 tool policy + path policy + counterfactual 的第一层治理 helper；新增 `src/llm_deep_governance.go` 收口 plan compiler + capability 的第一层治理 helper；新增 `src/llm_ifc_governance.go` 收口 IFC / quarantine / DOE 的第一层治理 helper；新增 `src/llm_deviation_governance.go` 收口 deviation repair / block / continue 的第一层治理 helper；新增 `src/llm_proxy_pipeline_test.go`、`src/llm_request_policy_test.go`、`src/llm_upstream_forward_test.go`、`src/llm_response_policy_test.go`、`src/llm_stream_finalize_test.go`、`src/llm_observability_test.go`、`src/llm_response_postprocess_test.go`、`src/llm_tool_governance_test.go`、`src/llm_deep_governance_test.go`、`src/llm_ifc_governance_test.go`、`src/llm_deviation_governance_test.go` 覆盖 request/response/stream finalize/observability/postprocess/tool governance/deep governance/ifc governance/deviation governance 基础行为
...
- [ ] **MCP 安全网关**（原 v27 规划，v_future_27：Agent 身份 + SMCP 五层安全 + 动态信任）
- [ ] **AI 安全运营副驾驶**（原 v28 规划，v_future_28：自然语言查询 + Benchmark 自动化 + Guardrail 市场）
...
> **v34.0 更新**: 从四层升级为五层。新增第零层（编译+全量单元测试），补充 v27-v34 实战经验。
> **核心铁律**: 测试失败了就说失败了，没跑测试就说没跑。事实是什么就报什么。
...
4. Quality — 五层质量保障（下方详述）
5. Deploy  — 部署测试服务器 + Dashboard 全页面走查
6. Release — 更新 README/CHANGELOG/config.example + Git tag
```

### 第零层：编译 + 全量单元测试（每次改动必跑）🆕

**方法**: `go build` + `go test -count=1 ./...`
**耗时**: ~70 秒（~1252 个测试函数）
**触发时机**: 每次代码改动后、提交前、部署前
**发现的问题类型**:
- 编译错误（类型不匹配、未使用 import、函数签名变更）
- 逻辑回归（已有功能被新代码破坏）
- 边界条件（nil pointer、空切片、零值）

**执行方式**: 本地 `go test`，不是 CI — 因为改一行就要跑，等 CI 太慢

**v34.0 实战数据**:
- P0 重构（proxy.go → trace.go + fixed_response.go）: 编译发现 unused import `"sync"`
- P1 重构（ServeHTTP 拆子函数）: 全量 69.9s 通过
- P2 重构（main.go → init_engines.go）: 全量 68.1s 通过
- 三步重构 1600+ 行代码搬迁，零回归失败

**铁律**:
- **go build 没过不准提交**
- **go test 没过不准说"完成"**
- **没跑 test 就说没跑，不准用"应该没问题"暗示通过了**
...
**执行方式**: 子 agent 批量 curl 全部 ~487 个 API 端点 + 异常输入 + 并发压测
...
- [ ] 🆕 conf.d YAML merge：同名 slice 覆盖不是追加（v32 教训）
- [ ] 🆕 INSERT OR IGNORE 后是否跟 UPDATE 同步内置数据（v31 教训）
- [ ] 🆕 go:embed 缓存：dist 文件变了是否 touch 了 dashboard.go（v34 教训）
...
**v27.0 实战补充**: 发现 6 大执行层断裂（proxy/llm_proxy/工具黑名单/路径策略/模板租户绑定），API 看起来写入成功但运行时完全不生效

**关键经验**:
- **map 遍历顺序不确定**: Go map 遍历不能用于降级选择
- **CAS 优于 Check-Then-Act**: 并发场景下 Lookup+Bind 必须原子化
- **同步等待+超时降级**: 比纯异步更适合首次请求（1500ms 是好的平衡点）
- 🆕 **`*bool` 区分零值和未传**: 用指针区分"传了 false"和"没传字段"（v33 教训）
- 🆕 **yaml.Unmarshal 对 slice 是替换**: 多个 conf.d 文件定义同一 slice 字段会互相覆盖，必须手动追加合并（v32 教训）
...
- [ ] 🆕 引擎开关 Dashboard UI vs config.yaml → 是否有单一真相源？（v32.15 教训）
- [ ] 🆕 固定返回 vs 上游转发 → upstream_id 校验是否正确放行？（v34 教训）
- [ ] 🆕 IM 回调签名参数位置（URL query vs body）→ 各通道插件是否统一处理？（v3.7 蓝信教训）
- [ ] 🆕 ~40 个引擎的 wire 注入 → 漏注入是否有编译期保护？（P2 重构 init_engines.go）
...
- 🆕 "config.yaml 写了 disabled_rules" × "proxy.go 写死 default" = 租户策略形同虚设（v27 六大断裂）
...
**22+ 验证场景**（v34.0 更新，随版本迭代扩展）:

| # | 场景 | 验证链路 | 版本 |
|---|------|---------|------|
| 1 | 正常对话 | 入站→LLM→Prompt追踪→出站, trace_id全链路 | v1+ |
| 2 | Prompt Injection | RuleEngine.Detect → block/warn | v1+ |
| 3 | 敏感信息泄露 | LLM tool_call → 出站PII检测 | v1+ |
| 4 | 异常行为模式 | 高频请求 + 高危工具注入 | v5+ |
| 5 | 蜜罐引爆 | ShouldTrigger → RecordTrigger → CheckDetonation → BLOCKED | v15+ |
| 6 | LLM多轮对话 | 5轮混合模型 + 7种工具 → 会话回放双向完整 | v13+ |
| 7 | Canary泄露+Budget | canary_leaked + budget_exceeded 标记 | v10+ |
| 8 | 多Agent画像 | 5种行为模式 → Agent风险分级 | v16+ |
| 9 | LLM响应过滤 | CheckResponse + 出站规则 + PII检测 | v10+ |
| 10 | 执行信封验证 | 全链路信封签名 + Merkle 批量验证 | v18+ |
| 11 | tool_calls 策略拦截 | LLM 请求危险工具 → block/warn | v20+ |
| 12 | 意图违反检测 | 超出声明范围 → 自动阻断 | v21+ |
| 13 | 污染传播追踪 | 敏感数据标记 → 跨 Agent 追踪 → 出站阻断 | v20+ |
| 14 | 🆕 CaMeL 执行计划 | 编译意图→Capability检查→偏差检测→修复或阻断 | v25+ |
| 15 | 🆕 IFC 信息流控制 | 变量注册→标签传播→Bell-LaPadula检查→隔离LLM路由 | v26+ |
| 16 | 🆕 租户策略闭环 | API Key识别→租户规则→禁用规则生效→工具黑名单拦截 | v27+ |
| 17 | 🆕 行业模板一键启用 | 选择行业→入站+LLM+出站三维度同时生效→F1验证 | v31+ |
| 18 | 🆕 反事实验证 | tool_call触发→反事实重放→归因报告→自适应策略 | v24+ |
| 19 | 🆕 路径策略 | 入站→步骤记录→序列/累计规则→降级→画像联动 | v23+ |
| 20 | 🆕 路由策略固定返回 | 命中策略→直接返回固定内容→IM主动回复→不转发上游 | v34+ |
| 21 | 🆕 Gateway WSS RPC | WSS连接→chat.send→等回复→models/channels/logs远程查询 | v29+ |
| 22 | 🆕 安全画像评分 | 5维评分→14表聚合→雷达图→Treemap→三级穿透 | v33+ |
...
每个页面（当前 50+ 页面/模态）必须通过浏览器操作验证（不是 curl）：
- [ ] 数据是否正确展示（和 API 返回一致）
- [ ] CRUD 操作是否正常（创建/编辑/删除）
- [ ] 搜索/筛选/分页是否工作
- [ ] 错误状态是否有合理提示（空数据/加载失败/权限不足）
- [ ] 跨页面跳转链接是否正确（审计→会话回放→用户画像）
- [ ] 🆕 引擎开关状态是否与 Settings 页面一致
- [ ] 🆕 GatewayMonitor WSS 连接状态是否准确
...
| 🆕 部署 | pkill 后旧进程被拉起 · /proc/PID/exe md5 确认 · go:embed 缓存不更新 |
| 🆕 IM集成 | 蓝信签名参数位置 · 解密后多事件拼接 · app_token 过期 |
...
| 层 | 方法 | 耗时 | 发现的典型问题 | Bug 严重度 | 就绪度提升 |
|----|------|------|---------------|-----------|-----------|
| 0 | 编译 + 全量单元测试 | ~70s | unused import、类型不匹配、逻辑回归 | P3-P4 | 前置保障 |
| 1 | API 黑盒 QA | ~10min/轮 | 返回码错误、字段缺失、并发计数 | P2-P4 | 0% → 55% |
| 2 | 数据流白盒审查 | ~1h | TOCTOU、conf.d覆盖、INSERT OR IGNORE | 🔴×3 🟡×4 | 55% → 75% |
| 3 | 设计矛盾分析 | ~15min | 亲和架空策略、租户断裂、引擎开关不一致 | 全 🔴 | 75% → 85% |
| 4 | E2E 实战验证 | ~1h | IM回复不到达、Dashboard空数据、部署旧进程 | 集成级 | 85% → 95% |

**核心结论**:
- **第零层是门票** — 编译+测试不过，后面都不用做
- **成本递增但严重度也递增** — 第一层便宜但找不到致命问题，第三层贵但找到的都是根基级问题
- **上一层找不到下一层的问题** — API QA 永远发现不了"亲和路由架空策略路由"
- **五层全跑才能达到 95%** — 只跑第零层+第一层就发版 = 55% 就绪度上线
- 🆕 **v34 实战验证**: 固定返回功能在 L0-L1 全部通过，但 L4 发现 IM 用户收不到回复（蓝信 API 格式问题），不到 E2E 永远发现不了
...
Phase 1 — 纯流量（不改上下游，只靠已有三条数据通道）:
  ...
  v31:     统一行业模板（40行业×三维度 + PII整合 + SSE流式拦截 + F1=1.0）         ✅
  v32.0-9: 架构优化 + Dashboard 演示级打磨（全链路调试 + SQLite优化 + 配置向导 + 43页面巡检 + UI统一）  ✅
...
产品定位演进:
  ...
  v30:     ✅ 质量硬化（模板统一 + AC 智能分级 + 架构拆分 + 链路修复 + 设置中心 + F1 基线）
  v31:     ✅ 统一行业模板（40行业×三维度 + PII整合 + SSE流式拦截 + F1=1.0 benchmark）
  v32.0-9: ✅ 架构优化 + Dashboard 演示级打磨（全链路调试 + SQLite批量写入 + 配置向导 + 43页面巡检 + UI统一）
  v32.10-15: ✅ 功能补全（红队建议 + Merkle审计 + 金丝雀轮换 + 报告定时 + 引擎开关统一）
...
产品灵魂：**龙虾卫士 — 让 AI Agent 不泄密、不越权、不失控**
  ...
  全局可见 → 安全画像(v33) + 5维评分 + Treemap + 三级穿透

商业化路径：
  内部验证（v30）✅ → AC智能分级 + F1基线 + 质量硬化
...
三个产品线:
  ...
  B — MCP 安全网关（市场热点卡位）   : v33+ SMCP/MCPShield/MCPSec
...
| **v31** | **统一模板 + Benchmark** | — | **40行业×三维度统一管理 + 500样本F1=1.0基线** |
| **v32** | **Conway 定律逆用** | — | **架构复杂度 = 运维复杂度，全链路调试+配置向导降低门槛** |
| v33+ | SMCP + MCPShield | arXiv:2602.01129 / 2602.14281 | MCP 安全是架构问题 → 网关层透明加固 |
| v34+ | Nash 均衡 | AgentDojo / AgentDyn | 安全均衡需要被设计 → AI 辅助全局视角 |
...
| v31 | 一个行业一个模板，**入站+LLM+出站三维度一键启用** — 不再分别管理三套规则 |
| v32 | **43 个页面全部正常不是终点** — 骨架屏/Toast/fade/排序高亮/配色tokens 才是演示级品质 |
| v33+ | MCP Server 说自己安全？**让龙虾卫士验证** — 信任要靠行为赢取不是声明 |
| v34+ | 安全运营不是看 Dashboard，是**跟安全助手对话** |
...
| SMCP (2602.01129) | 2026.02 | — | v33+ | MCP 五层安全透明叠加 | 中 |
| MCPShield (2602.14281) | 2026.02 | — | v33+ | MCP Tool 动态信任评分 | 小 |
| MCPSec (2601.17549) | 2026.01 | — | v33+ | 协议级加固 (52.8%→12.4% ASR) | 中 |
| MCP Landscape (ACM TOSEM) | 2025 | 420 | v33+ 需求输入 | 16 种 MCP 威胁场景需求清单 | — |
| AgentDojo (NeurIPS 2024) | 2024 | — | v34+ | Benchmark 自动化测试 | 小 |
| AgentDyn (2602.03117) | 2026.02 | — | v34+ | 动态 Benchmark 测试 | 小 |
