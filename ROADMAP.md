...
- [ ] **v36.4 LLM Proxy 管线化重构**
  ...
  - 当前进展：已抽出 `src/llm_request_preprocess.go`，先收拢 trace/taint/request-path 三个 request preprocess helper；新增 `src/llm_request_policy.go` 收口请求侧规则检测与立即阻断分支；新增 `src/llm_upstream_forward.go` 收口上游请求构造、header 复制、审计上下文构建；新增 `src/llm_response_policy.go` 收口非流式 + SSE 缓冲模式的响应规则阶段；新增 `src/llm_stream_finalize.go` 收口 SSE 尾部 rewrite/event formatting helper；新增 `src/llm_observability.go` 收口 envelope sealing 与 audit async hooks；新增 `src/llm_response_postprocess.go` 收口 taint propagation / reversal / cache-store 响应后处理 helper；新增 `src/llm_proxy_pipeline_test.go`、`src/llm_request_policy_test.go`、`src/llm_upstream_forward_test.go`、`src/llm_response_policy_test.go`、`src/llm_stream_finalize_test.go`、`src/llm_observability_test.go`、`src/llm_response_postprocess_test.go` 覆盖 request/response/stream finalize/observability/postprocess 基础行为
  - 重点保障：rewrite、taint reversal、reasoning_content fallback、tool policy、counterfactual、SSE 尾包行为完全可回归测试
  - 测试：`src/llm_proxy_pipeline_test.go` / `src/llm_request_policy_test.go` / `src/llm_upstream_forward_test.go` / `src/llm_response_policy_test.go` / `src/llm_stream_finalize_test.go` / `src/llm_observability_test.go` / `src/llm_response_postprocess_test.go`
