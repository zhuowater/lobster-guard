...
### v5.0 — 可观测性 + 运维增强
- [x] 结构化日志（JSON 格式可选，便于 ELK/Loki 日志采集）
- [x] 请求追踪（trace_id 贯穿全链路 · X-Trace-ID header · 审计日志/Dashboard 集成）
- [x] 审计日志归档（SQLite → gzip JSON 归档 · 自动/手动 · 归档文件管理 API）
- [x] Dashboard 实时监控大屏（QPS 柱状图 · 攻击实时流 · 关键指标大字 · 3 秒刷新）
- [x] 运维命令行增强（`-check-config` · `-dump-routes` · `-dump-rules` · `-version`）

**当前状态**: v5.0 · 21 个源文件 ~19300 行 · 408 个测试 · 3 个依赖

---

## 计划

### v5.0 — 可观测性 + 运维增强 (已在上方标记完成)
