...
- [x] 入站+出站规则均支持 priority/message (112 → 129 tests)
...
### v3.8 — 多 Bot 亲和路由（已完成 2026-03-16）
- [x] 路由键从 `sender_id` 升级为 `(sender_id, app_id)` 复合键
- [x] 同一用户访问不同 Bot 可路由到不同龙虾容器
- [x] 用户部门/显示名属性存储
- [x] 批量绑定 API（按部门、按列表）
- [x] 路由统计 API（按 Bot/上游/部门 分布）
- [x] Dashboard 全新路由管理面板（筛选/搜索/批量操作/统计栏）
- [x] 数据库 schema 自动迁移（旧单键表 → 新复合键表）
- [x] 完全向后兼容（app_id 可选，旧 API 不变）
