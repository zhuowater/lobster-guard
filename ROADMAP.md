...
> **当前版本：v32.0** · 270+ commits · Go ~96,400 行 + 测试 ~38,500 行 · Vue ~30,300 行 · 1252 测试 · 365+ API · 50 页面 · 75 Vue 文件 · 53 引擎 · 5 依赖
>
> 更新时间：2026-03-29
...
- [ ] v32.10 **出站行业模板**
  - 与入站/LLM 模板对称，`IndustryTemplate` 已有 `outbound_rules` 字段（v31.0 定义但未填充）
  - 金融出站：账号/卡号/SWIFT 泄露、交易金额外发
  - 政务出站：涉密文件/红头文号/公民身份证外发
  - 医疗出站：病历号/基因数据/HIV 状态外发
  - 复用 v31.0 统一模板的 DB `enabled` 全局开关 + 租户绑定机制
  - Dashboard: Rules.vue 行业模板卡片新增出站规则 section
...
- [ ] v32.15 **CaMeL 三引擎 Dashboard 独立开关**
...
Phase 1 — 纯流量（不改上下游，只靠已有三条数据通道）:
  ...
  v32.10+: 功能补全（出站模板 + 红队闭环 + Merkle UI + 金丝雀轮换 + 报告自动化 + 引擎开关）
------- v32.0 当前版本 · v32.10+ 进行中 -------
...
产品定位演进:
  ...
  v32.10+: 功能补全（出站模板 + 红队闭环 + Merkle UI + 金丝雀轮换 + 报告自动化 + 引擎开关）
  v33+:    IM 安全前端模式 + MCP 安全网关 + AI 安全副驾驶（Phase 3 生态扩展 → 商业化）
...
商业化路径：
  ...
  产品化（v32.0-9）✅ → 配置向导 + SQLite优化 + Dashboard演示级打磨
  功能补全（v32.10+）→ 出站模板 + 红队闭环 + Merkle验证 + 报告自动化
  → 商业化（v33+）→ IM前端模式 + MCP安全网关 + 私有化部署、数据不出域
