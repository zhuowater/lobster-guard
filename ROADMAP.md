...
### v31.x — P2 改进 · 功能补全 · 自动化运营
> v30 质量硬化后的边际改进：成本最低的提升、功能补全、自动化运营闭环
> 原则：每个改动都是已有功能的**自然延伸**，不引入新架构概念
...
- [ ] v31.0 **出站行业模板**
  ...
  - 复用入站模板的 DB `enabled` 全局开关 + 租户绑定机制
  - Dashboard: 出站模板管理（与入站模板同一个 Tab，分 section 展示）

- [ ] v31.1 **红队→规则演化闭环**
  - 红队引擎（v14.2）发现的绕过模式 → 自动生成新规则**建议**（不自动添加）
  - Dashboard: 规则建议队列（审核→接受/拒绝→热更新到 RuleEngine）
  - 演化引擎（v20）与红队引擎对接：红队报告 → 演化建议 → 管理员审批 → 规则生效
  - 闭环：攻击→检测→绕过→红队发现→建议→审批→加固→再测

- [ ] v31.2 **Merkle 审计验证 UI**
  - `merkle.go` 已实现 Merkle 树批量签名，Dashboard 缺验证入口
  - 新增验证页面：选择时间范围 → 批量验证 → 展示完整性报告
  - 异常标红：被篡改的审计记录高亮展示
  - 导出验证报告（PDF/HTML），支持合规审计场景

- [ ] v31.3 **金丝雀令牌自动轮换**
  - 当前 `canary_token` 在 config 中写死，一旦泄露攻击者可以规避
  - 自动轮换机制：每 N 小时（可配）自动生成新的 canary token
  - 轮换日志：记录每次轮换时间 + 旧 token 泄露检测结果
  - Dashboard: 金丝雀管理卡片（当前 token 状态、上次轮换、泄露检测历史）

- [ ] v31.4 **报告定时自动生成**
  - `report.go`（v12.0）能手动生成报告，但没有 cron 触发
  - 新增定时任务：每周一 9:00 自动生成周报 → 蓝信群推送
  - 报告内容：安全事件摘要 + 健康分趋势 + 规则命中 TOP10 + 建议
  - 与告警通知（v5）复用 Webhook 通道

- [ ] v31.5 **CaMeL 三引擎 Dashboard 独立开关**
  - v28 做了 6 引擎开关，但计划编译器/能力引擎/偏差检测缺少独立 Dashboard 开关
  - 补齐到 Settings 引擎 Tab 中（v30.4 的自然延伸）

### v32.x — 架构级优化 · 性能与可维护性
> 解决全链路审计发现的架构级问题：规则去重、SQLite 瓶颈、配置简化
> 原则：**不加功能，只减复杂度**

- [ ] v32.0 **入站/LLM 规则去重**
  - 问题：入站 AC 和 LLM 规则有大量重复 pattern（如 "ignore previous instructions" 两层都有）
  - 分析哪些 pattern 在两层都出现，评估去重策略：
    - 方案 A：入站层只保留"快速拦截"（明确恶意），LLM 层保留"上下文感知"（需要 LLM 对话上下文）
    - 方案 B：共享规则库，每条规则标记适用层（inbound/llm/both）
  - 去重后测试：确保不降低检测率（用 v30.5 的 P/R/F1 基线对比）

- [ ] v32.1 **SQLite 写入优化**
  - 问题：65 张表在一个 WAL 模式 SQLite 里，高并发写可能成为瓶颈
  - 分离热/冷数据：
    - 热 DB（主库）：audit_log, llm_calls, gateway_log, honeypot_triggers（高频写入）
    - 冷 DB（分析库）：attack_chains, behavior_profiles, reports（低频聚合查询）
  - 写入批量化：审计日志攒 N 条或 T 秒后批量 INSERT（减少 WAL 刷写频率）
  - 监控：Dashboard 显示 SQLite WAL 大小、写入 QPS、查询延迟

- [ ] v32.2 **配置简化 · 新手引导**
  - 问题：config.yaml 字段超过 200 个，新用户上手门槛极高
  - 分层配置：
    - Level 0（快速开始）：5 个必填字段（上游地址、管理端口、token、通道类型、加密密钥）
    - Level 1（基础安全）：+10 个常用字段（入站检测、出站审计、LLM Proxy）
    - Level 2（高级功能）：全部字段
  - Dashboard 配置向导：Step-by-step UI，每步只展示该级别的字段
  - `config.yaml.example` 按级别分 section，注释标注优先级

- [ ] v32.3 **三层检测规则重叠分析工具**
  - 开发分析工具：输入一条测试文本 → 显示在入站/LLM/出站三层分别命中了哪些规则
  - 可视化重叠矩阵：哪些规则在多层重复、哪些只在一层
  - 自动建议：基于命中数据推荐去重方案
  - API: `POST /api/v1/debug/detect-all-layers` — 全链路检测调试端点

- [ ] v32.4 **高级引擎实战验证**
  - 问题：v23-v26 的 CaMeL/Fides 理论完善，但 142 上没有真实 tool_call 流量验证
  - 构造 tool_call 模拟流量：覆盖计划编译、能力检查、偏差检测、IFC 标签传播
  - 每个引擎单独验证：开启→注入测试流量→检查命中→关闭→对比
  - 输出：每个引擎的实战验证报告（有效性、误报率、性能影响）
...
产品定位演进:
  ...
  v31:     P2 改进（出站模板 + 红队闭环 + Merkle UI + 金丝雀 + 报告自动化）
  v32:     架构优化（规则去重 + SQLite 分离 + 配置简化 + 引擎实战验证）
  v33+:    MCP 安全网关 + AI 安全副驾驶（Phase 3 生态扩展 → 商业化）
...
商业化路径：
  ...
  → 功能补全（v31）→ 出站模板 + 报告自动化 + 红队闭环
  → 产品化（v32）→ 配置向导 + 性能优化 + 文档打磨
  → 商业化（v33+）→ 私有化部署、数据不出域、行业模板市场
...
| v31 | 帕累托改进 | — | P2 改进，边际成本最低的功能补全 |
| v32 | Conway 定律逆用 | — | 架构复杂度 = 组织复杂度，简化架构即简化运维 |
...
| v32 | **50 个引擎 65 张表 200 字段 — 复杂度本身就是安全风险**，简化即加固 |
