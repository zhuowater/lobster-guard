...
- [ ] **出站行业模板**（v33+）— 与入站模板对称，按行业定义出站检测规则
- [ ] **红队→规则演化闭环**（v33+）— 红队发现绕过 → 自动生成规则建议 → 审批 → 热更新
- [ ] **Merkle 审计验证 UI**（v33+）— Dashboard 验证入口 + 完整性报告 + 异常标红
- [ ] **金丝雀令牌自动轮换**（v33+）— 定时生成新 canary token + 泄露检测
- [ ] **报告定时自动生成**（v33+）— 每周自动周报 → 蓝信群推送
- [ ] **CaMeL 三引擎 Dashboard 独立开关**（v33+）— 补齐到 Settings 引擎 Tab
...
Phase 1 — 纯流量（不改上下游，只靠已有三条数据通道）:
  ...
  v32:     架构优化 + Dashboard 演示级打磨（全链路调试 + SQLite优化 + 配置向导 + 43页面巡检 + UI统一）  ✅
------- v32.0 当前版本 · 以下为规划 -------
...
产品定位演进:
  ...
  v32:     ✅ 架构优化 + Dashboard 演示级打磨（全链路调试 + SQLite批量写入 + 配置向导 + 43页面巡检 + UI统一）
...
商业化路径：
  ...
  产品化（v32）  ✅ → 配置向导 + SQLite优化 + Dashboard演示级打磨
